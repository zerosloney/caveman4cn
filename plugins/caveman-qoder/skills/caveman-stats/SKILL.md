...
description: >
  ...
  Reads directly from the Qoder session logs — no AI estimation.
...
本技能由 `UserPromptSubmit` hook `hooks/user-prompt.js` 提供，它调用 `hooks/caveman-stats.js` 读取 Qoder 会话记录。

会话日志定位策略（按优先级）：
1. **hook stdin 的 `transcript_path` 字段**（最可靠）——Qoder 的 hook 通用字段直接指向当前会话日志文件，`/caveman-stats` 时优先用它。
2. **候选目录探测**——`~/.qoder/{projects,sessions,logs,transcripts,history}/`，覆盖 Claude-Code 风格（Qoder CLI 衍生自该架构，`projects/<encoded>/<id>.jsonl`）及常见备选名。
