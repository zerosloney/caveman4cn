#!/usr/bin/env node
// caveman init (Trae build) — drop the always-on caveman activation rule
// into a target repo's AGENTS.md. Trae reads root-level AGENTS.md on every
// session (Trae rules docs confirm AGENTS.md compatibility), so this makes
// caveman mode auto-activate with no per-session prompt. Idempotent. Safe
// to re-run.
//
// Usage:
...
// (Trae's discovery convention). Cursor/Windsurf/Cline/Copilot/OpenClaw
...
function help() {
  console.log(`caveman init (CodeBuddy) — drop always-on caveman rule into AGENTS.md
...
Target:
  AGENTS.md   appended (created if absent). CodeBuddy reads this every session.
...
function main() {
  ...
  console.log(`🪨 caveman init (CodeBuddy) — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
