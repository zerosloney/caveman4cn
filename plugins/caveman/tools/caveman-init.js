#!/usr/bin/env node
// caveman init (Qwen Code build) — drop the always-on caveman activation rule
// into a target repo's AGENTS.md. Qwen Code reads AGENTS.md on every session,
...
// (Qwen Code's discovery convention; QWEN.md is also recognized but AGENTS.md
// is the cross-host standard). Cursor/Windsurf/Cline/Copilot/OpenClaw targets
// dropped — they belong to other host ecosystems.
...
function help() {
  console.log(`caveman init (Qwen Code) — drop always-on caveman rule into AGENTS.md
...
function main() {
  ...
  console.log(`🪨 caveman init (Qwen Code) — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
