#!/usr/bin/env node
// caveman init (ZCode build) — drop the always-on caveman activation rule
// into a target repo's AGENTS.md. ZCode reads AGENTS.md on every session,
...
// Cursor/Windsurf/Cline/Copilot/OpenClaw targets dropped — they belong to
// other host ecosystems.
...
function help() {
  console.log(`caveman init (ZCode) — drop always-on caveman rule into AGENTS.md
...
Target:
  AGENTS.md   appended (created if absent). ZCode reads this every session.
...
function main() {
  ...
  console.log(`🪨 caveman init (ZCode) — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
