#!/usr/bin/env node
// caveman-stats.js — shared aggregator for /caveman-stats (Qwen Code build).
//
// Qwen Code does not publicly document where it writes session transcripts.
// This module probes several candidate locations under ~/.qwen/ and falls
...
// Qwen changes its transcript schema.
//
...
// Candidate transcript roots Qwen Code might use. The first that exists wins.
// Sources for these guesses:
//   - issue #628: ~/.qwen/sessions/<ID>.json  (note: .json, not .jsonl)
//   - issue #362: ~/.qwen/logs/openai          (JSON-formatted provider logs)
//   - Claude-Code-style: ~/.qwen/projects/<encoded>/<id>.jsonl
//   - generic fallbacks
...
function candidateRoots() {
  return [
    ...
    path.join(homeDir, '.qwen', 'logs', 'openai'),
    path.join(homeDir, '.qwen', 'transcripts'),
 ...
 * (single-blob, Qwen sessions/<ID>.json per issue #628).
...
function sumUsage(transcriptFiles) {
  ...
  for (const file of transcriptFiles) {
    ...
    if (file.endsWith('.json')) {
      // Single-blob transcript (Qwen sessions/<ID>.json). Parse once, then walk
...
function formatStats(stats) {
  if (!stats.found) {
    return 'No Qwen Code session log found yet.\n' +
      'Probed ~/.qwen/{projects,sessions,logs,transcripts,history}/ and the\n' +
